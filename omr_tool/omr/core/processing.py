...
from omr_tool.omr.exceptions import *
import sys
...
def get_binary_code_from_outer_box(greyscale_outer_box, h1, h2, w1, w2, r1, r2, min_dist, num_circles=None):
    ...
    code = get_code_from_binary_circles(code_box, min_dist, r1, r2, num_circles=num_circles)
    print('INFO: code: {}'.format(code))
    return code


def get_inner_boxes(greyscale_outer_box, height, width,
                    bottom_left_corners):
    ...
    for corner_h, corner_w in bottom_left_corners:
        ...
        box_cnt = {'h1': int(corner_h_abs - box_height),
                   'h2': int(corner_h_abs),
                   'w1': int(corner_w_abs),
                   'w2': int(corner_w_abs + box_width)}
        inner_box_cnts.append(box_cnt)
    inner_boxes = [greyscale_outer_box[cnt['h1']:cnt['h2'], cnt['w1']:cnt['w2']] for cnt in inner_box_cnts]
...
def get_code_from_binary_circles(image, min_dist, min_radius, max_radius, num_circles=None):
    code_circles = cv2.HoughCircles(image, cv2.HOUGH_GRADIENT, 1,
                                    min_dist,
                                    param1=150,
                                    param2=5,
    ...
    sorted_circles = sorted(code_circles, key=lambda circle: circle[0] ** 2 + circle[1] ** 2, reverse=True)
    ...
    # plt.show()
    if num_circles:
        assert len(code_circles) == num_circles, 'found {} circles whilst trying ' \
...
def process_images_folder(input_folder, form_design_path=None, omr_mode='attendance', output_folder=None):
    if omr_mode == 'exam':
        temp_module = importlib.import_module('omr_tool.omr.exam_marksheet.processing')
    elif omr_mode == 'attendance':
        temp_module = importlib.import_module('omr_tool.omr.attendance_register.processing')
    else:
...
def get_outer_box(original_image, desired_portrait=True):
    ...
    if not portrait == desired_portrait:
        raise OmrValidationException('outer box not found with correct orientation')
