...
assert len(
    sys.argv
) == 2, 'usage: python3.6 process_exam_marksheets.py <path to folder of folders>'
input_input_folder = sys.argv[1]
print('processing exam marksheets in child folders of {}'.format(
    input_input_folder))
...
for input_path in Path(input_input_folder).iterdir():
    ...
    try:
        process_exam_marksheet_folder(
            str(input_path), config, str(output_folder))
