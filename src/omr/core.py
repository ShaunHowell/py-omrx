...
def process_images_folder(input_folder,
                          ...
                          output_folder=None):
    if omr_mode == 'exam':
        temp_module = importlib.import_module('omr.exam_marksheet')
    elif omr_mode == 'attendance':
        temp_module = importlib.import_module('omr.attendance_register')
    else:
        print(
...
def process_boxes(inner_boxes,
    ...
    for box_no, inner_box in enumerate(inner_boxes):
        ...
        try:
            ...
            print(
                'Couldn\'t extract answers from box {}'.format(box_no + 1),
                file=sys.stderr)
